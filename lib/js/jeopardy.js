...
function resizeAnswerModal() {
    ...
    questionCenterPadding = ($('#question-modal-body').height() - ($('#question-image').height() + $('#question').height()))/4;
    $('#question').css('padding-top', questionCenterPadding);

    let questionSplit = document.getElementById('question').innerText.split('<br>', 2);
    if (questionSplit.length == 2)
        $('#question').html(questionSplit[0] + '<br><br>' + questionSplit[1]);
}
